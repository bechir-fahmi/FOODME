...
export class TokenService implements OnDestroy {
  ...
  constructor(private store: LocalStorageService, private factory: TokenFactory, protected jwtHelper: JwtHelperService) {}

  private get token(): BaseToken | undefined {
    if (!this._token) {
      this._token = this.factory.create(this.store.get(this.key), this.jwtHelper);
    }
  ...
  private save(token?: Token): void {
    ...
    if (!token) {
      ...
      const value = { access_token: token.access_token, token_type: 'Bearer', exp:token.exp,
      };
