...
namespace Workflow
{
    public class WorkflowInit
    {
        ...
        public WorkflowInit(IRestaurantService restaurantService)
        {
        ...
        private static WorkflowRuntime InitWorkflowRuntime()
        {
            ...
            OrderRules orderRules = new OrderRules();
            OrderActionsProvider orderActionsProvider = new OrderActionsProvider(_restaurantService);

            var runtime = new WorkflowRuntime()
                .WithBuilder(builder)
                .WithPersistenceProvider(dbProvider)
                .WithRuleProvider(orderRules)
                .WithActionProvider(orderActionsProvider)
                .EnableCodeActions()
